...
const VisibilityPage: React.FC<VisibilityPageProps> = ({ nextStep, prevStep, areToastsEnabled }) => {
  const { profile, loading: profileLoading, organisation: userOrganisations } = useProfile(); // MODIFIED: Get userOrganisations as OrganisationEntry[]
  const {
      ...
      <div className="space-y-4 flex-grow">
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            {userOrganisations && userOrganisations.length > 0 && ( // MODIFIED: Check userOrganisations directly
              <Button
                variant="outline"
                className={cn(
                  "h-auto py-3 flex items-center justify-start gap-3 text-lg",
